...
const StarCommentButtonFill: React.FC<{
  ...
  <OverlayTrigger
    placement="auto"
    overlay={<Tooltip id="editTooltip">Remove star from comment</Tooltip>}
  >
    <button
      id="starFillBtn"
      type="button"
...
const StarCommentButtonUnFill: React.FC<{
  ...
  <OverlayTrigger
    placement="auto"
    overlay={<Tooltip id="editTooltip">Give comment a star!</Tooltip>}
  >
    <button
      id="starUnfillBtn"
      type="button"
...
const StarCommentButtonNoUser: React.FC<{ stars: number }> = ({ stars }) => (
  <OverlayTrigger
    placement="auto"
    overlay={<Tooltip id="editTooltip">Login or register to like</Tooltip>}
  >
    <button
      id="starUnAuthBtn"
      type="button"
...
const CommentElement: React.FC<{
  ...
  user: LoggedUser;
  createdBy: string;
  setCommentToEdit: React.Dispatch<
    React.SetStateAction<ListComment | undefined>
  >;
  ...
  return (
    <Accordion className="mb-2">
      <Card className="commentCard">
          ...
          <Col className="text-right align-self-end mt-3 mr-4">
            <div style={{ lineHeight: "90%" }}>
              ...
              <small style={{ color: "darkgray" }}>
                {comment.edited
                  ? `(Edited ${new Date(comment.edited).toLocaleString(
                      "default",
        ...
        <Row
          style={
            user && comment.user._id === user.id ? {} : { marginTop: "-30px" }
          }
        >
        ...
        <Accordion.Collapse eventKey={comment._id}>
          <Card.Body>
            <div style={{ marginTop: "-70px" }}>
              <div
                style={
                  comment.replies.length !== 0 && user
                    ? {
                        borderLeft: "1px solid #c8cbcf",
                        marginLeft: "5%",
                        marginBottom: "20px",
                      }
                    : {}
