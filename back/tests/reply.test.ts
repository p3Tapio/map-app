...
describe('Reply can be deleted', () => {
  ...
  test('...by the user who wrote it when authenticated', async () => {
    ...
    await api.delete(`/api/reply/deletereply/${commentsInDb[0]._id}`).set({ 'token': login.body.token }).send({replyId: replyRes.body._id, listId:  commentRes.body[0].list});
    commentRes = await api.get(`/api/comment/comments/${listsInDb[0]._id}`);
    expect(commentRes.body[0].replies).toHaveLength(0);
  })
  test('...by the user who created the list,', async () => {
    ...
    await api.delete(`/api/reply/deletereply/${commentsInDb[0]._id}`).set({ 'token': login_2.body.token }).send({replyId: replyRes.body._id, listId:  commentRes.body[0].list});
    commentRes = await api.get(`/api/comment/comments/${listsInDb[0]._id}`);
    expect(commentRes.body[0].replies).toHaveLength(0);
  ...
  test('...not without authentication', async () => {
    ...
    await api.delete(`/api/reply/deletereply/${commentsInDb[0]._id}`).set({ 'token': '' }).send({replyId: replyRes.body._id, listId:  commentRes.body[0].list}).expect(401);
    commentRes = await api.get(`/api/comment/comments/${listsInDb[0]._id}`);
    expect(commentRes.body[0].replies).toHaveLength(1);
  })
  test('...not by who has not created the list or written the reply', async () => {
    const login = await api.post('/api/user/login/').send(user);
    const commentsInDb = await Comment.find({});
    const listsInDb = await List.find({})
    ...
    const login_2 = await api.post('/api/user/login/').send(anotherUser);
    await api.delete(`/api/reply/deletereply/${commentsInDb[0]._id}`).set({ 'token': login_2.body.token}).send({replyId: replyRes.body._id, listId:  commentRes.body[0].list}).expect(401);
    commentRes = await api.get(`/api/comment/comments/${listsInDb[0]._id}`);
    expect(commentRes.body[0].replies).toHaveLength(1);
  })
})
